...
**--irqbalance-config-restore-file**="": Determines if CRI-O should attempt to restore the irqbalance config at startup with the mask in this file. Use 'disable' value to disable the restore flow entirely. (default: /etc/sysconfig/orig_irq_banned_cpus)
